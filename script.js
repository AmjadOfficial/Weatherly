let weather = {
    ...
    displayWeather: function (data) {
        const { name, sys } = data; // Extracting country information from the data
        const { icon, description } = data.weather[0];
        const { temp, humidity } = data.main;
        const { speed } = data.wind;
        const country = sys.country; // Accessing the country code from the 'sys' object
        
        // Displaying city and country with comma separation
    ...
    search: function () {
        const city = document.querySelector(".search-bar").value.trim(); // Trim to remove leading/trailing whitespace
